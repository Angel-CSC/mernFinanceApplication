...
const Row3 = () => {
  ...
  const productsWithIds = products.map(product => ({
    id: product._id, // Add unique identifier
    ...product
  }));

  const transactionsWithIds = transactions.map(transaction => ({
    id: transaction._id, // Add unique identifier
    ...transaction
  }));
  ...
  const productColumns = [
    ...
    },
    {
      ...
      headerName: "Expense",
      flex: 0.5,
      renderCell: (params: GridCellParams) => `$${convertStringToNumber(params.value)}`,
    },
    {
      field: "price",
      headerName: "Price",
  ...
  const pieChartData = useMemo(() => {
    if(kpi){
      const totalExpenses = parseFloat((kpi[0].totalExpenses).substring(1));
      return Object.entries(kpi[0].expensesByCategory).map(
        ([key, value]) => {
          const categoryValue = convertStringToNumber(value);
          return [
            {
              name: key,
              value: categoryValue
    ...
    <>
        <DashboardBox gridArea="g">
          ...
          <Box
            ...
            p="0 0.5rem"
            height="75%"
            //this will be targetting the class of a child, in this case DataGrid when using &
            sx={{
              "& .MuiDataGrid-root": {
        ...
        <DashboardBox gridArea="i">
          {
            /* 
            <BoxHeader title="Expense Breakdown By Category" sideText="something something 4 percent"/>
            <FlexBetween
              mt="0.5rem"
              gap="0.5rem"
              p="0 1rem"
        ...
        <Typography margin="0 1rem" variant="h6">
          something something something this information is so imprtant here is something about 
          it this is very very very good or very very very bad something in between maybe idk kys
        </Typography>
        </DashboardBox>
    </>
  )
